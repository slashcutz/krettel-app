<x-admin-layout>
    ...
    <div class="max-w-4xl bg-card border border-border rounded-xl overflow-hidden">
        ...
        <form action="{{ route('admin.playlists.update', $playlist->id) }}" method="POST" class="p-4 sm:p-6">
            @csrf
            @method('PUT')
            ...
            <div class="mt-8 pt-6 border-t border-border flex flex-col-reverse sm:flex-row gap-3 sm:justify-end sm:space-x-4">
                <a href="{{ route('admin.playlists.index') }}" class="px-4 py-2.5 border border-border rounded-lg text-white hover:bg-secondary transition-colors text-center w-full sm:w-auto">Cancel</a>
