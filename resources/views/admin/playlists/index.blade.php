            ...
            <div class="p-4">
                <div class="flex items-center gap-3">
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-muted truncate">{{ $playlist->user->name ?? 'System' }}</p>
                    </div>
                    @if($playlist->visibility == 'public')
                        <span class="px-2 py-1 text-xs font-semibold rounded-full bg-success/20 text-success flex-shrink-0">Public</span>
                    @else
                        <span class="px-2 py-1 text-xs font-semibold rounded-full bg-warning/20 text-warning flex-shrink-0">Private</span>
                    @endif
                </div>
                <div class="mt-3 flex items-center justify-between">
                    <span class="text-xs text-muted">{{ $playlist->created_at->format('M d, Y') }}</span>
                    <div class="flex items-center gap-5">
                        <a href="{{ route('admin.playlists.show', $playlist->id) }}" class="text-sm font-medium text-primary hover:underline">View Videos</a>
                        <a href="{{ route('admin.playlists.edit', $playlist->id) }}" class="text-sm font-medium text-primary hover:underline">Edit</a>
                        <form id="playlist-delete-{{ $playlist->id }}" action="{{ route('admin.playlists.destroy', $playlist->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="button" onclick="confirmDelete('playlist-delete-{{ $playlist->id }}', '{{ addslashes($playlist->name) }}')" class="text-sm text-muted hover:text-white">Delete</button>
                        </form>
                    </div>
                </div>
            </div>
            @empty
                <p class="px-4 py-8 text-center text-sm text-muted">No playlists found.</p>
            @endforelse
        </div>
