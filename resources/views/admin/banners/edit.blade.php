        ...
        <form action="{{ route('admin.banners.update', $banner->id) }}" method="POST" class="p-4 sm:p-6">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="url" name="image_url" id="image_url" value="{{ old('image_url', $banner->image_url) }}" class="bg-secondary border border-border text-white text-sm rounded-lg focus:ring-primary focus:border-primary block w-full p-2.5" placeholder="https://...">
                </div>

                <div>
                    <label for="link_url" class="block text-sm font-medium text-white mb-2">Link URL</label>
                    <input type="url" name="link_url" id="link_url" value="{{ old('link_url', $banner->link_url) }}" class="bg-secondary border border-border text-white text-sm rounded-lg focus:ring-primary focus:border-primary block w-full p-2.5" placeholder="https://...">
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="status" class="bg-secondary border border-border text-white text-sm rounded-lg focus:ring-primary focus:border-primary block w-full p-2.5">
                        @foreach(['active', 'inactive', 'draft'] as $opt)
                            <option value="{{ $opt }}" {{ old('status', $banner->status) == $opt ? 'selected' : '' }}>{{ ucfirst($opt) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
