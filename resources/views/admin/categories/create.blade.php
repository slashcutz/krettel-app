<x-admin-layout>
    ...
    <div class="max-w-4xl bg-card border border-border rounded-xl overflow-hidden">
        ...
        <form action="{{ route('admin.categories.store') }}" method="POST" class="p-4 sm:p-6">
            @csrf
            ...
            <div class="mt-8 pt-6 border-t border-border flex flex-col-reverse sm:flex-row gap-3 sm:justify-end sm:space-x-4">
                ...
                <button type="submit" class="bg-primary hover:bg-red-600 text-white px-4 py-2.5 rounded-lg transition-colors font-medium w-full sm:w-auto">Create Category</button>
            </div>
        </form>
    </div>
</x-admin-layout>
