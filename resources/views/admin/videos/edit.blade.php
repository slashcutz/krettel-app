<x-admin-layout>
    ...
    <div class="max-w-4xl bg-card border border-border rounded-xl overflow-hidden">
        ...
        <form action="{{ route('admin.videos.update', $video->id) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="mt-8 pt-6 border-t border-border flex justify-end space-x-4">
                <a href="{{ route('admin.videos.index') }}" class="px-4 py-2 border border-border rounded-lg text-white hover:bg-secondary transition-colors">Cancel</a>
