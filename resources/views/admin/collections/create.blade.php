            ...
            <div class="space-y-6">
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name') }}" class="bg-secondary border border-border text-white text-sm rounded-lg focus:ring-primary focus:border-primary block w-full p-2.5" @input="document.getElementById('slug').value = $event.target.value.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,'')" required>
                    ...
                    <div>
                        ...
                        <input type="text" name="slug" id="slug" value="{{ old('slug') }}" class="bg-secondary border border-border text-white text-sm rounded-lg focus:ring-primary focus:border-primary block w-full p-2.5" required>
                ...
                <div>
                    ...
                    <input type="text" name="terabox_image" id="terabox_image" value="{{ old('terabox_image') }}" placeholder="https://... or terabox://remote/path" class="bg-secondary border border-border text-white text-sm rounded-lg focus:ring-primary focus:border-primary block w-full p-2.5">
                ...
                <div>
                    ...
                    <select name="video_ids[]" id="video_ids" multiple size="8" class="bg-secondary border border-border text-white text-sm rounded-lg focus:ring-primary focus:border-primary block w-full p-2.5">
                        @forelse($videos as $video)
                            <option value="{{ $video->id }}" {{ collect(old('video_ids', []))->contains($video->id) ? 'selected' : '' }}>
